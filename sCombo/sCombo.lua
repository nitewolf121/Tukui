...
if IsAddOnLoaded("ElvUI") then
    E, C, L, DB = unpack(ElvUI) -- Import: E - functions, constants, variables; C - config; L - locales
else
    T, C, L = unpack(Tukui) -- Import: T - functions, constants, variables; C - config; L - locales
end
ElvDPS_target:DisableElement('CPoints')
for i = 1, 5 do ElvDPS_target.CPoints[i]:Hide() end
local Options = {
    x = E.Scale(3),
    comboWidth = E.Scale(55),
    comboHeight = E.Scale(10),
...
--sPowerBG:CreatePanel(nil, (Options.comboWidth * 5) + (Options.x * 5) - Options.x, Options.comboHeight, "TOPLEFT", sCombo[1], "BOTTOMLEFT", 0, -Options.x)
